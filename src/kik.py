...
def main_menu():
    while True:
        ...
        if player.sign == 'X':
            ...
            bot.sign = 'X'
        game.button_display('START', game.center[0] + 90, game.center[1] + 110, 80, 60, game_loop, args=(True,))
        game.events()
        game.display_update()
...
def end(board_list: list, state: str, win: list):
    while True:
        ...
        if state == 'Remis':
            game.message_display(state, 'Mistral', 40, game.colors['blue'], (game.center[0] + len(state) // 2, game.center[1] + 100))
        elif state == 'Wygrana':
            game.message_display(state, 'Mistral', 40, game.colors['green'], (game.center[0] + len(state) // 2, game.center[1] + 100))
        elif state == 'Przegrana':
            game.message_display(state, 'Mistral', 40, game.colors['red'], (game.center[0] + len(state) // 2, game.center[1] + 100))
        ...
        game.button_display('MENU', game.center[0] - 140, game.center[1] + 110, 80, 60, main)
        game.button_display('PONÓW', game.center[0] + 90, game.center[1] + 110, 80, 60, main)
        game.events()
        game.display_update()
